...
int main()
{
    FILE *image, *outputImage, *lecturas,*imageantes;
    ...
    unsigned char xx[54]; //Creamos la variable para recorrer al ancho de la imagen
    int cuenta = 0;

    for(int i=0; i<54; i++) {
        xx[i] = fgetc(image);
        fputc(xx[i], outputImage);   //Copia cabecera a nueva imagen
        fputc(xx[i], imageantes);
    }
