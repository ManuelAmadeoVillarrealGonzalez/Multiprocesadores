        ...
        #pragma omp for nowait
            for(int xx = 0; xx < alto; xx++){
                for(int yy = 0; yy < ancho; yy++){
                    ...
                    for(int x = yy; x < alto && x < yy + 7; x++){
                        for(int y = xx; y < ancho && y < xx + 7; y++){
                            avgB += ptr[(x) * ancho + (y) ];
                    ...
                    fputc(ptr[(yy) * ancho + (xx) ] = avgB, outputImage);
                    fputc(ptr[(yy) * ancho + (xx) + 1] = avgG, outputImage);
                    fputc(ptr[(yy) * ancho + (xx) + 2] = avgR, outputImage);
